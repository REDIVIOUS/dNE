...
class DNN(nn.Module):
    def __init__(self, N, k):
        ...
        self.l1 = nn.Linear(self.flow_no, 256)
        self.l2 = nn.Linear(256, 512)
        self.l3 = nn.Linear(512, 256)
        self.l4 = nn.Linear(256, self.flow_no * self.k)

    def forward(self, x):
        x = x.view(-1, self.flow_no)
        x = F.relu(self.l1(x))
        x = F.relu(self.l2(x))
        x = F.relu(self.l3(x))
        return self.l4(x)

class RUN_DNN(object):
    ...
    def my_dataloader(self):
        ...
        for i in range(len(traffic_data)):
            traffic_load.append((np.array(traffic_data_normalized[i]), np.array(traffic_data[i])))
    ...
    def run(self, alpha=0, learning_rate=0.01):
        seed = 76 # 0.01 for abilene; 0.001 for geant
        ...
        # the model
        model = DNN(N=self.N, k=self.k).to(device)
