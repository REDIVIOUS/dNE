...
class CNN(nn.Module):
    def __init__(self, N, output_size, kernel_size):
        ...
        self.conv_layer1 = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=16, kernel_size=self.kernel_size, stride=1, padding=self.padding),
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=True)
        )
        layer1_size = math.ceil(self.N/2)
        self.conv_layer2 = nn.Sequential(
            nn.Conv2d(in_channels=16, out_channels=32, kernel_size=self.kernel_size, stride=1, padding=self.padding),
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=True)
        )
        layer2_size = math.ceil(layer1_size/2)
        ...
        layer3_size = math.ceil(layer2_size/2)

        self.fc = nn.Linear(64*layer3_size*layer3_size,self.output_size)
...
class RUN_CNN(object):
    ...
    def run(self, alpha=0, learning_rate=0.001):
        ...
        # the model
        model = CNN(N=self.N, output_size=self.k * self.flow_no, kernel_size=3).to(device)
